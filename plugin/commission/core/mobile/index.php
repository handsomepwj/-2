...
class Index_EweiShopV2Page extends CommissionMobileLoginPage 
{
	public function main() 
	{
		...
		$men = pdo_fetch('select count(*) from' . tablename('ewei_shop_member') . 'where agentid = :agentid and agentlevel = 3' , array(':agentid' =>$member['id']));
		$men1 = pdo_fetch('select count(*) from' . tablename('ewei_shop_member') . 'where agentid = :agentid and agentlevel = 4' , array(':agentid' =>$member['id']));

		if($men['count(*)'] >= '3' && $member["agentlevel"] == '3'){
			$member['agentlevel'] = 4;
			pdo_update('ewei_shop_member', array('agentlevel' => $member['agentlevel']), array('id' => $member['id']));
		}
		if(($men1['count(*)'] == '8'||($men['count(*)'] == '5' && $men1['count(*)'] == '3')) && $member["agentlevel"] == '4'){
			$member['agentlevel'] = 5;
			pdo_update('ewei_shop_member', array('agentlevel' => $member['agentlevel']), array('id' => $member['id']));
		}


//echo ("<script>console.log('".json_encode($men1['count(*)'])."');</script>");
//echo ("<script>console.log('".json_encode($men['count(*)'])."');</script>");
