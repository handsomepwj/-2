	...
	class DividendModel extends PluginModel 
	{
		...
		public function getInfo($openid, $options = NULL) 
		{
			...
			if( in_array("total", $options) ) 
			{
				...
				$mb = pdo_fetch("select id from" . tablename("ewei_shop_member") . "where agentid =:agentid and isheads = 1 and headsstatus = 1",array( ":agentid" => $member["id"]));
				$mb1 = pdo_fetch("select id from" . tablename("ewei_shop_member") . "where agentid =:agentid and isheads = 1 and headsstatus = 1",array( ":agentid" => $mb["id"]));
				$mb2 = pdo_fetch("select id from" . tablename("ewei_shop_member") . "where agentid =:agentid and isheads = 1 and headsstatus = 1",array( ":agentid" => $mb1["id"]));
				$order_all1 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 0 and uniacid = :uniacid", array( ":headsid" => $mb["id"], ":uniacid" => $_W["uniacid"] ));
				$order_all2 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 0 and uniacid = :uniacid", array( ":headsid" => $mb1["id"], ":uniacid" => $_W["uniacid"] ));
				$order_all3 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 0 and uniacid = :uniacid", array( ":headsid" => $mb2["id"], ":uniacid" => $_W["uniacid"] ));
				$order_all4 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and agentid=:agentid and status >= 0 and uniacid = :uniacid", array( ":agentid" => $mb["id"], ":uniacid" => $_W["uniacid"] ));
				$order_all5 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and agentid=:agentid and status >= 0 and uniacid = :uniacid", array( ":agentid" => $mb1["id"], ":uniacid" => $_W["uniacid"] ));
				$order_all6 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and agentid=:agentid and status >= 0 and uniacid = :uniacid", array( ":agentid" => $mb2["id"], ":uniacid" => $_W["uniacid"] ));
				...
				if( !empty($order_all1) ) 
				{
					...
					{
						$divedend = iunserializer($v["dividend"]);
						$dividend_total += $divedend["dividend_price1"];
					}
				}
				if( !empty($order_all2) ) 
				{
					foreach( $order_all2 as $k => $v ) 
					{
						$divedend = iunserializer($v["dividend"]);
						$dividend_total += $divedend["dividend_price2"];
					}
				}
				if( !empty($order_all3) ) 
				{
					foreach( $order_all3 as $k => $v ) 
					{
						$divedend = iunserializer($v["dividend"]);
						$dividend_total += $divedend["dividend_price3"];
					}
				}
				if( !empty($order_all4) ) 
				{
					foreach( $order_all4 as $k => $v ) 
					{
						$divedend = iunserializer($v["dividend"]);
						$dividend_total += $divedend["dividend_price1"];
					}
			...
			if( in_array("ok", $options) ) 
			{
				...
				$mb1 = pdo_fetch("select id from" . tablename("ewei_shop_member") . "where agentid =:agentid and isheads = 1 and headsstatus = 1",array( ":agentid" => $mb["id"]));
				$mb2 = pdo_fetch("select id from" . tablename("ewei_shop_member") . "where agentid =:agentid and isheads = 1 and headsstatus = 1",array( ":agentid" => $mb1["id"]));
				//$order_ok = pdo_fetchall("select id,agentid,headsid,price,deductcredit2,dispatchprice,dividend from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and status >= 0 and uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"] ));
				...
				$divedend_order_ok1 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $mb["id"], ":uniacid" => $_W["uniacid"] ));
				$divedend_order_ok2 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $mb1["id"], ":uniacid" => $_W["uniacid"] ));
				$divedend_order_ok3 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $mb2["id"], ":uniacid" => $_W["uniacid"] ));
				$divedend_order_ok4 = pdo_fetchall("select id,agentid,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and status >= 3 and dividend_status=0 and agentid=:agentid and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"] ,":agentid" =>$mb["id"]));
				$divedend_order_ok5 = pdo_fetchall("select id,agentid,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and status >= 3 and dividend_status=0 and agentid=:agentid and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"] ,":agentid" =>$mb1["id"]));
				$divedend_order_ok6 = pdo_fetchall("select id,agentid,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and status >= 3 and dividend_status=0 and agentid=:agentid and (" . $time . "-finishtime > " . $day_times . ") and uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"] ,":agentid" =>$mb2["id"]));
				...
				if( !empty($divedend_order_ok1) ) 
				{
					
					foreach( $divedend_order_ok1 as $k => $v ) 
					{		
						$dividend = iunserializer($v["dividend"]);
						$dividend_ok += $dividend["dividend_price1"];
					}
						
				}
				if( !empty($divedend_order_ok2) ) 
				{
					
					foreach( $divedend_order_ok2 as $k => $v ) 
					{		
						$dividend = iunserializer($v["dividend"]);
						$dividend_ok += $dividend["dividend_price2"];
					}
						
				}
				if( !empty($divedend_order_ok3) ) 
				{
					
					foreach( $divedend_order_ok3 as $k => $v ) 
					{		
						$dividend = iunserializer($v["dividend"]);
						$dividend_ok += $dividend["dividend_price3"];
					}
						
				}
				if( !empty($divedend_order_ok4 )){
						foreach( $divedend_order_ok4 as $k => $v ) 
					{		
						$dividend = iunserializer($v["dividend"]);
						$dividend_ok += $dividend["dividend_price1"];
					}
				}
				if( !empty($divedend_order_ok5 )){
						foreach( $divedend_order_ok5 as $k => $v ) 
					{		
						$dividend = iunserializer($v["dividend"]);
						$dividend_ok += $dividend["dividend_price2"];
					}
				}
				if( !empty($divedend_order_ok6 )){
						foreach( $divedend_order_ok6 as $k => $v ) 
					{		
						$dividend = iunserializer($v["dividend"]);
						$dividend_ok += $dividend["dividend_price3"];
					}
			...
			if( in_array("lock", $options) ) 
			{
				...
				$check_apply_lock1 = pdo_fetchall("select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = :headsid and status >= 3 and dividend_status=0 and (" . $time . "-finishtime <= " . $day_times . ") and uniacid = :uniacid", array( ":headsid" => $mb["id"], ":uniacid" => $_W["uniacid"] ));
				...
				if( !empty($check_apply_lock1) ) 
				{
					foreach( $check_apply_lock1 as $k => $v ) 
					{
						$dividend = iunserializer($v["dividend"]);
						$dividend_lock += $dividend["dividend_price1"];
					}
				}
		...
		public function checkOrderConfirm($orderid = "0") 
		{
			...
			$order = pdo_fetch("select id,openid,ordersn,goodsprice,agentid,paytime,officcode,headsid,dispatchprice,deductcredit2,price,deductcredit2 from " . tablename("ewei_shop_order") . " where id=:id and status>=0 and uniacid=:uniacid limit 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));
			if( empty($order) ) 
			{
				return NULL;
			}
			...
			$member = m("member")->getMember($openid);
			//当前定单的队长
			$openid1 = $member["headsid"];
			$member1 = m("member")->getMember($openid1);
			//当前订单队长的上级
			$openid2 = $member1["agentid"];
			$member2 = m("member")->getMember($openid2);
			//当前队长的上上级
			$openid3 = $member2["agentid"];
			$member3 = m("member")->getMember($openid3);
			//当前队长的上上上级
			$openid4 = $member3["agentid"];
			$member4 = m("member")->getMember($openid4);
			if( empty($member) ) 
			{
				return NULL;
			}
			...
			$divedend["dividend_ratio"] = $set["ratio"];
				...
				if($member1["agentid"] ==$member2["id"]){
			$divedend["dividend_price1"] = number_format(((($order["price"] + $order["deductcredit2"]) - $order["dispatchprice"]) * ($set["ratio"] + 5.2)) / 100, 2);
			$divedend["dividend_ratio1"] = $set["ratio"] + 5.2;
			}
			if($member2["agentid"] ==$member3["id"]){
			$divedend["dividend_price2"] = number_format(((($order["price"] + $order["deductcredit2"]) - $order["dispatchprice"]) * ($set["ratio"] + 3.25)) / 100, 2);
			$divedend["dividend_ratio2"] = $set["ratio"] + 3.25;
			}
			if($member3["agentid"] ==$member4["id"]){
			$divedend["dividend_price3"] = number_format(((($order["price"] + $order["deductcredit2"]) - $order["dispatchprice"]) * ($set["ratio"] + 1.95)) / 100, 2);
			$divedend["dividend_ratio3"] = $set["ratio"] + 1.95;
			}
