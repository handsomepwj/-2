...
$mb = pdo_fetch("select id from" . tablename("ewei_shop_member") . "where agentid =:agentid and isheads = 1 and headsstatus = 1",array( ":agentid" => $member["id"]));
		...
		$divedend_order_ok4 = pdo_fetchall("select id,agentid,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from " . tablename("ewei_shop_order") . " where 1 and headsid = 0 and agentid=:agentid and status >= 3 and dividend_status=0 and uniacid = :uniacid", array( ":uniacid" => $_W["uniacid"],":agentid" =>$mb["id"] ));

		//echo ("<script>console.log('".json_encode($order_ok)."');</script>");	
		 echo ("<script>console.log('".json_encode($divedend_order_ok4)."');</script>");	
		// echo ("<script>console.log('".json_encode($mb["id"])."');</script>");	
